...
if ($result->num_rows > 0) :
  ...
  while ($row = $result->fetch_assoc()) :
    ...
    $data=array('id'=>$row['product_id'],'name'=>$row['name'],'image'=>$row['image'],'price'=>$row['price'],'quantity'=>$row['quantity'],'total'=>$row['total']);
    $total+=$row['total'];
    $status='pending';
...
$query="INSERT INTO orders (cartdata,total,status,ordertime) VALUES ('$cartdata','$total','$status',NOW())";
    $result = mysqli_query($connection, $query) or die(mysqli_error($connection));
